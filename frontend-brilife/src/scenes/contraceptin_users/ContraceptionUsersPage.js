...
import { deleteAll, findAll } from "../../actions/contraception_user";
import styles from "../styles.js";
import Swal from "sweetalert2";

class ContraceptionUsersPage extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  onAdd = () => {
    this.props.history.push("/contraception-users/add");
  };
  ...
  onRowClick = (rowData) => {
    this.props.history.push(`/contraception-users/${rowData[0]}`);
  };
    ...
    return (
      <div>
        ...
        >
          ...
          >
            New ContraceptionUser
          </Button>
        </ButtonGroup>
        <div className={classes.table}>
          <MUIDataTable
            title={"ContraceptionUser List"}
            data={!loading ? data : []}
            columns={columns}
            options={options}
          />
...
ContraceptionUsersPage.propTypes = {
...
const mapStateToProps = (state) => ({
  data: state.ContraceptionUsers.data,
  error: state.ContraceptionUsers.error || state.ContraceptionUsers.error,
  loading: state.ContraceptionUsers.loading || state.ContraceptionUsers.loading,
  deleteData: state.ContraceptionUsers.data,
  deleteError: state.ContraceptionUsers.error
});
...
export default withStyles(styles, { withTheme: true })(
  connect(mapStateToProps, mapDispatchToProps)(ContraceptionUsersPage)
);
